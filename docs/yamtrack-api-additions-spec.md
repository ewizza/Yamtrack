...
**Docker verification (2026-09-11)**: pushed to `origin/yamtv-api-additions` (`e46228e5`), rebuilt the image (`docker build -t ewizza/yamtrack:yamtv-api-additions .`), recreated `yamtrack_dashboard` (`docker compose up -d --force-recreate yamtrack`) - no new migrations, clean startup, no errors in logs. Hit the real `GET /api/search` against Eric's actual `ewizza` account (queried from inside the running container to reach `localhost:8000` directly, token never printed to any log or output): `?query=blues+brothers` returned real TMDB movie results (`The Blues Brothers`, tmdb_id `525`, plus several related titles) with `tracked_status: null` for all of them (none are tracked); `?query=reacher` returned the real tracked show (`Reacher`, tmdb_id `108978`, `tracked_status: "In progress"`, matching its actual watchlist status) alongside an untracked spin-off (`Neagley`, `tracked_status: null`) - confirming the batched per-media-type tracked-status lookup works against real data, not just mocked tests. Both an empty `?query=` and a missing `query` param cleanly 400'd (`"query must not be empty."`) without ever reaching TMDB. No errors in the container logs before or after.
